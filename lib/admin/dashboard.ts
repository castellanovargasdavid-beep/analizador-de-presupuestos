...
import { estimates, faqs, leads, pricingRules, seoGuides, seoQuestions } from "@/db/schema";
...
export async function getDashboardStats(): Promise<DashboardStats> {
  const [[{ total: totalEstimates }], leadsRows, [activeRule], [{ total: publishedGuides }], [{ total: publishedQuestions }], [{ total: activeFaqs }]] =
    await Promise.all([
